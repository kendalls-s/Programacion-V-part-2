// ========================================
// TIPOS DE IDENTIFICACIÓN - JAVASCRIPT
// ========================================
...
const API_URL = 'https://localhost:7021/api/TipoIdentificacion';
        ...
        html += `
            <div class="card-bordered">
                ...
                <div class="card-bordered-footer">
                    <button type="button" class="btn-sm btn-info" onclick="editarTipoIdentificacion(${t.id})">
...
async function cargarTiposIdentificacion() {
    ...
    try {
        ...
        document.getElementById('listaTiposIdentificacion').innerHTML = `
...
function abrirModalCrear() {
    document.getElementById('tituloModal').textContent = 'Nuevo Tipo de Identificación';
        ...
        if (response.status === 404) {
            resultado.innerHTML = `
                <div class="alert alert-warning">
                    <i class="bi bi-exclamation-triangle"></i>
                    No se encontró ningún tipo de identificación con ID ${id}
                </div>
            `;
            return;
        }
...
async function guardarTipoIdentificacion() {
    ...
    try {
        ...
        if (response.ok) {
            mostrarAlerta(isEdit ? 'Tipo de identificación actualizado' : 'Tipo de identificación creado', 'success');
...
async function editarTipoIdentificacion(id) {
    ...
    try {
        ...
        document.getElementById('tituloModal').textContent = 'Editar Tipo de Identificación';
...
async function eliminarTipoIdentificacion() {
    ...
    try {
        ...
        if (response.ok) {
            cerrarModalEliminar();
            mostrarAlerta('Tipo de identificación eliminado correctamente', 'success');
            cargarTiposIdentificacion();
        } else {
            const error = await response.json();
